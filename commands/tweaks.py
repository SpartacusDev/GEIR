...
class Packages(menus.Menu):
    ...
    def create_embed(self, page: int=0) -> Embed:
        ...
        embed_message.add_field(name="Dependencies", value=f"{', '.join([f'`{dependency}`' for dependency in self.packages[page]['depends']])}")
        ...
        embed_message.add_field(name="Tags", value=f"{', '.join([f'`{tag}`' for tag in self.packages[page]['tag']])}" if not self.packages[page]["tag"] == "" else "None")
...
class Tweaks(commands.Cog):
    ...
    async def on_message(self, message: Message):
        ...
        embed_message.add_field(name="Dependencies", value=f"{', '.join([f'`{dependency}`' for dependency in first_result['depends']])}")
        ...
        embed_message.add_field(name="Tags", value=f"{', '.join([f'`{tag}`' for tag in first_result['tag']])}" if not first_result["tag"] == "" else "None")
